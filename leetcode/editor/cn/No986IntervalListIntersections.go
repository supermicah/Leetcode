...
func main() {
	no986Print("%+v", intervalIntersection([][]int{{0, 2}, {5, 10}, {13, 23}, {24, 25}}, [][]int{{1, 5}, {8, 12}, {15, 24}, {25, 26}}))
}

func no986Print(format string, params ...interface{}) {
...
func intervalIntersection(firstList [][]int, secondList [][]int) [][]int {
	...
	for i < len(firstList) && j < len(secondList) {
		...
		s := intervalIntersectionMax(s1, s2)
		e := intervalIntersectionMin(e1, e2)
...
func intervalIntersectionMax(x, y int) int {
	if x > y {
		return x
	}
	return y
...
func intervalIntersectionMin(x, y int) int {
	if x > y {
		return y
	}
	return x
